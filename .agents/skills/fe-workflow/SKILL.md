...
7. 요청받으면 변경 의도를 나타내는 커밋 메시지와 `dev` 대상 PR 제목·본문을 간단히 제안한다. 미완료 작업은 완료된 것처럼 쓰지 않는다. PR 본문은 `## 작업 내용` 섹션에 글머리 기호로 실제 변경 사항만 나열한다. 관련 이슈 연결·스크린샷·환경 변수 변경처럼 `CONTRIBUTING.md`가 요구하는 항목은 실제로 해당할 때만 덧붙인다.
