...
3. 구현은 사용자가 요청한 단위만 진행한다. 커밋·푸시·PR 생성은 각각 별도 요청이 있을 때만 실행한다.
4. 코드 변경 후 관련 검사를 실행하고 결과를 구분해 보고한다. 이 저장소의 기본 검사는 `pnpm typecheck`, `pnpm lint`, 포맷 검사이며, 빌드는 실행 가능한 환경에서 확인한다.
5. 이번 요청에서 실제 변경한 파일별로 `#### 파일 경로` 제목 아래 핵심 작업을 짧은 글머리 기호로 요약한다. 이전 작업이나 미변경 파일을 이번 변경으로 소개하지 않는다.
6. 요청받으면 변경 의도를 나타내는 커밋 메시지와 `dev` 대상 PR 제목·본문을 간단히 제안한다. 미완료 작업은 완료된 것처럼 쓰지 않는다.
