...
### 통합 브랜치 (상위 이슈가 여러 서브 이슈로 나뉠 때)

- 상위 이슈 하나가 여러 서브 이슈 PR로 나뉘어 진행되고, 전체가 함께 동작해야 의미가 있는
  경우 `dev`에서 통합 브랜치를 분기한다.
- 각 서브 이슈의 작업 브랜치는 `dev`가 아닌 이 통합 브랜치를 대상으로 PR을 생성하고 병합한다.
- 상위 이슈에 속한 모든 서브 이슈가 병합되고 전체 흐름이 동작하는 것을 확인한 뒤에만
  통합 브랜치를 `dev`로 병합한다.
- 통합 브랜치도 작업 브랜치 네이밍 규칙(`feature/이슈번호` 등)을 따르되, 상위 이슈 번호를
  사용한다.
...
- 완료 결과와 테스트가 독립적인가?
- 따로 리뷰·병합할 가치가 있는가?
- 외부 의존성 때문에 완료 시점이 달라질 수 있는가?

### PR을 나눌 기준

- 한 번에 리뷰하고 검증하기에 큰가?
- 위험도가 크게 다른 변경이 섞여 있는가?

### 커밋을 나눌 기준

- 하나의 변경 의도로 설명되는가?
- 그 커밋까지만 체크아웃해도 빌드·테스트가 가능한가?
- 독립적으로 되돌릴 수 있는가?
