...
export const MeetingTranscript = ({ segments, isRecording, isPaused }: MeetingTranscriptProps) => {
  ...
  const { containerRef, unseenCount, handleScroll, scrollToLatest } = useTranscriptAutoScroll(
    segments.length,
  );
    ...
    <div className="relative flex min-h-0 flex-1 flex-col">
      ...
      {unseenCount > 0 && (
        <button
          ...
          className="absolute bottom-3 left-1/2 flex -translate-x-1/2 items-center gap-1.5 rounded-full bg-brand-600 px-4 py-2 text-xs font-semibold text-white shadow-[0_8px_24px_rgba(20,34,56,0.18)]"
        >
          <ArrowDown aria-hidden="true" className="size-3.5" strokeWidth={2.4} />새 녹취
        </button>
      )}
    </div>
  );
